...
(function () {
    ...
    function parseDuration(durationStr) {
        if (durationStr === 'Pending...') return Infinity;
        if (durationStr === 'Failed') return -Infinity;
        return parseFloat(durationStr);
    }
    function updateRequests() {
      performance.getEntriesByType("resource").forEach(res => {
          const absoluteUrl = res.name;
          if (!uniqueUrls.has(absoluteUrl)) {
                ...
                originalRequestData.push(requestEntry);
            }
      });
    }
        ...
        XMLHttpRequest.prototype.open = function (method, url) {
           ...
           if (!uniqueUrls.has(absoluteUrl)) {
                ...
                this.addEventListener('load', () => {
                    ...
                    const originalEntry = originalRequestData.find(entry => entry.url === absoluteUrl);
                    if(originalEntry){
                      originalEntry.duration = requestEntry.duration;
                    }
                    renderList();
                });
    ...
    function renderList() {
        let dataToRender = [...requestData]; 

        
         dataToRender = dataToRender.filter(req => {
        ...
        if (sortState === 'asc') {
            dataToRender.sort((a, b) => parseDuration(a.duration) - parseDuration(b.duration));
        } else if (sortState === 'desc') {
            dataToRender.sort((a, b) => parseDuration(b.duration) - parseDuration(a.duration));
        } else {
            dataToRender = originalRequestData.filter(req => dataToRender.some(filteredReq => filteredReq.url === req.url));
        }
          const listHtml = dataToRender.map(req => `
    ...
    function initEventListeners() {
     ...
     document.getElementById('sort-button').addEventListener('click', () => {
            if (sortState === 'default') {
                sortState = 'asc';
                document.getElementById('sort-button').textContent = 'Sort ↑';
